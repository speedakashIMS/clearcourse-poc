import { ArrowLeftIcon } from "@heroicons/react/24/outline";
import MapLocations from "../components/MapLocations";
import IntroductionHeader from "../components/IntroductionHeader";
import Buttons from "../components/Buttons";
import Spacer from "../components/Spacer";

export default function MapLocationsPage() {
  const locations = [
    {
      title: 'Location Title',
      description: 'Lorem ipsum dolor sit amet',
      imageSrc: 'https://placehold.co/100x100?text=Logo',
      pins: [
        { text: 'Lorem ipsum', iconName: 'pin' },
        { text: 'Lorem ipsum', iconName: 'envelope' },
        { text: 'Lorem ipsum', iconName: 'phone' },
      ],
      pinIconName: 'pin',
    },
    {
      title: 'Location Title',
      description: 'Lorem ipsum dolor sit amet',
      imageSrc: 'https://placehold.co/100x100?text=Logo',
      pins: [
        { text: 'Lorem ipsum', iconName: 'pin' },
        { text: 'Lorem ipsum', iconName: 'envelope' },
        { text: 'Lorem ipsum', iconName: 'phone' },
      ],
      pinIconName: 'pin',
    },
    {
      title: 'Location Title',
      description: 'Lorem ipsum dolor sit amet',
      imageSrc: 'https://placehold.co/100x100?text=Logo',
      pins: [
        { text: 'Lorem ipsum', iconName: 'pin' },
        { text: 'Lorem ipsum', iconName: 'envelope' },
        { text: 'Lorem ipsum', iconName: 'phone' },
      ],
      pinIconName: 'pin',
    },
  ];

  return (
    <div>
      <div className="bg-grey-200 py-32">
        <div className="container">
          <div className="">
            <a
              href="/"
              className="inline-flex items-center gap-10 text-grey-600 hover:text-grey-950 mb-20 transition-colors"
            >
              <ArrowLeftIcon className="h-20 w-20" />
              Back to Home
            </a>
            <h1 className="text-headings-h1 font-bold text-grey-950 mb-20">Map Locations Component</h1>
            <p className="text-body-default text-grey-600 mb-24">Map with location cards and buttons</p>
          </div>
        </div>
      </div>

      <section className="py-40">
        <IntroductionHeader
          title="Section Header"
          description="Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
        />
        <Spacer height={40} />
        <MapLocations
          mapIframeUrl="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d243646.00425560038!2d-74.25196116091908!3d40.69767006536973!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x89c250b4033d2f3f%3A0x97c0b1b0b1b0b1b0!2sNew%20York%2C%20NY%2C%20USA!5e0!3m2!1sen!2sus!4v1700000000000!5m2!1sen!2sus"
          locations={locations}
        />
        <Spacer height={40} />
        <Buttons buttons={[
          { text: 'Button', variant: 'dark', href: '#' },
          { text: 'Button', variant: 'dark', href: '#' },
          { text: 'Button', variant: 'dark', href: '#' },
        ]}
        />  
      </section>

    </div>
  );
}

